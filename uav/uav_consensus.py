...
from observer.PredefinedTimeDO import predefined_time_do as pdt_do
...
from controller.PdT_FNTSMC import pdt_fntsmc_param, pdt_fntsmc_consensus, pdt_fntsmc
...
class usv_consensus:
    def __init__(self,
                 ...
                 ctrl_att_param: Union[fntsmc_param, rfntsmc_param, pdt_fntsmc_param],  # parameters of attitude controller
                 ctrl_pos_param: Union[fntsmc_param, rfntsmc_param, ftpd_param, pdt_fntsmc_param],  # parameters of position controller
                 ...
                 obs_att: Union[rd3, pdt_do],  # observer of attitude loop
                 obs_pos: Union[rd3, pdt_do],  # observer of position loop
                 is_ideal: bool  # exist disturbance of not
                 ):
        self.uav = UAV(uav_param)  # UAV
        
        if isinstance(ctrl_att_param, fntsmc_param):
            self.ctrl_att = fntsmc(ctrl_att_param)  # 内环控制器
        else:
            self.ctrl_att = pdt_fntsmc(ctrl_att_param)  # 内环控制器
        
        if isinstance(ctrl_pos_param, ftpd_param):
            self.ctrl_pos = ftpd(ctrl_pos_param)        # 外环控制器
        elif isinstance(ctrl_pos_param, rfntsmc_param):
            self.ctrl_pos = rfntsmc_consensus(ctrl_pos_param)   # 外环控制器
        elif isinstance(ctrl_pos_param, pdt_fntsmc_param):
            self.ctrl_pos = pdt_fntsmc_consensus(ctrl_pos_param)    # 外环控制器
        else:
        ...
        self.Q_vel = np.array([0.05, 0.05, 0.05])
    ...
    def cal_Lambda_eta(self,
        ...
        res = np.array((self.d + self.b) * dotdot_nu + self.b * dotdot_eta_d)
    ...
    def calculate_cost(self, eta_d: np.ndarray, nu: np.ndarray, dot_eta_d: np.ndarray, dot_nu: np.ndarray):
        ...
        # cost_vel = np.dot((self.uav.dot_eta() - dot_eta_d - dot_nu) ** 2, self.Q_vel)
        cost_vel = 0.
        self.cost += (cost_pos + cost_vel)
    
    def reset(self, _uav_param, _ctrl_att_param, _ctrl_pos_param):
        self.uav.reset_with_param()
        self.ctrl_att.reset_with_new_param(_ctrl_att_param)
        self.ctrl_pos.reset_with_new_param(_ctrl_pos_param)
        self.obs_att.reset()
        self.obs_pos.reset()
        
        self.data_record.reset()

...
class bs_fntsmc_consensus(bs_fntsmc):
	...
	def cal_consensus_e_de(self,
						   ...
						   global_nu: np.ndarray,
						   global_dot_nu: np.ndarray,
						   eta_d: np.ndarray,
						   ...
						   B: float,
						   A: np.ndarray):
		Lambda = B * eta_d + (D + B) * nu
		for eta_j, a_j, nu_j in zip(global_eta, A, global_nu):
			Lambda += a_j * (eta_j - nu_j)
		self.consensus_e = (D + B) * eta - Lambda

		dot_Lambda = B * dot_eta_d + (D + B) * dot_nu
		for dot_eta_j, a_j, dot_nu_j in zip(global_dot_eta, A, global_dot_nu):
			dot_Lambda += a_j * (dot_eta_j - dot_nu_j)
	...
	def control_update_outer_consensus(self,
									   ...
									   global_nu: np.ndarray,  # 所有无人机的偏移量
									   global_dot_nu: np.ndarray,  # 所有无人机的偏移量的导数
									   eta_d: np.ndarray,  # 参考 eta
									   dot_eta_d: np.ndarray,  # 参考 eta 的一阶导
									   eta: np.ndarray,  # eta
									   dot_eta: np.ndarray,  # eta 一阶导
									   nu: np.ndarray,  # 该无人机的 偏移量
									   dot_nu: np.ndarray,  # 该无人机的偏移量的导数
									   D: float,  # 入度
									   B: float,  # 通信
									   A: np.ndarray,  # 临接
									   kt: float,  # uav 风阻系数
		...
		self.cal_consensus_e_de(global_eta, global_dot_eta, global_nu, global_dot_nu, eta_d, dot_eta_d, nu, dot_nu, eta, dot_eta, D, B, A)

...
class pdt_fntsmc_consensus(pdt_fntsmc):
    def __init__(self,
        ...
        super(pdt_fntsmc_consensus, self).__init__(param, a_s, b1_s, b2_s, b3_s, Ts, k1_s, a_c, b1_c, b2_c, b3_c, Tc, k1_c, k2, dim, dt)
    ...
    def control_update_outer_consensus(self,
                                       ...
                                       e_m: np.ndarray = np.array([2, 2, 2]).astype(float),
                                       de_m: np.ndarray = np.array([1.5, 1.5, 1.5]).astype(float)):
        e_eta = np.clip(consensus_e, -e_m, e_m)
        dot_e_eta = np.clip(consensus_de, -de_m, de_m)
        ...
        u1 = -(d + b) * kt / m * dot_eta - Lambda_eta + M_rho
        u2 = (d + b) * obs
        ...
        # print(self.control_out_consensus)

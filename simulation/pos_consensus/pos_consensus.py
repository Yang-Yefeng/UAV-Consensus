...
if __name__ == '__main__':
    ...
    while g_t < g_tm - g_dt / 2:
        ...
        for i in range(UAV_NUM):  # 对于每一个无人机
            ...
            uncertainty_i = consensus_un[g_N, 6 * i: 6 * (i + 1)]
            ref_i, dot_ref_i, _, _ = ref_uav(g_t, ref_amplitude, ref_period, ref_bias_a, ref_bias_phase)
            bias_i = bias[i]
            dot_bias_i = np.zeros(4)

            eta_d_i = ref_i + bias_i  # eta 表示外环，d表示参考，i表示无人机编号
            dot_eta_d_i = dot_ref_i + dot_bias_i  # dot 表示一阶导数，eta表示外环，d表示参考，i表示无人机编号
            ...
            e_rho = uavs[i].uav.rho1() - uavs[i].rho_d
            de_rho = np.dot(uavs[i].uav.W(), uavs[i].uav.rho2()) - uavs[i].dot_rho_d

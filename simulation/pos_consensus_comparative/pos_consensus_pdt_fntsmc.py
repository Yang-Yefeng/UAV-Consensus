...
for i in range(g_v['uav_num']):
    obs_in = pdt_do(T0=np.array([1, 1, 1]).astype(float),
...
def cal_g_obs_2nd_dynamic(g_dd_nu: np.ndarray):
    ...
    for p in range(g_v['uav_num']):
        _g_obs[p][:] = uavs[p].obs_pos.delta
        _dy[p][:] = -uavs[p].uav.kt / uavs[p].uav.m * uavs[p].uav.dot_eta() + uavs[p].uav.A() - g_dd_nu[p]
...
if __name__ == '__main__':
    while g_v['g_t'] < g_v['g_tm'] - g_v['dt'] / 2:
        ...
        for i in range(g_v['uav_num']):  # 对于每一个无人机
            ...
            if USE_OBS_OUT:
                # -uav.kt / uav.m * uav.dot_eta() + uav.A()
                syst_dynamic_i = -uavs[i].uav.kt / uavs[i].uav.m * uavs[i].uav.dot_eta() + uavs[i].uav.A()
                obs_eta_i = uavs[i].obs_pos.observe(de=uavs[i].uav.dot_eta(), dy=syst_dynamic_i)
            else:
                obs_eta_i = np.zeros(3)
            
            '''2.2 consensus control update'''
            uavs[i].cal_consensus_e(g_eta, nu, nu[i], eta_d_i, sat=False, thresh=np.array([2.5, 2.5, 2.8]))  # 计算 e
            ...
            uavs[i].ctrl_pos.control_update_outer_consensus(d=uavs[i].d,
                                                            ...
                                                            obs=obs_eta_i,
                                                            g_obs=g_obs,
                                                            e_m=np.array([5., 5., 3.]), # 0.5
                                                            de_m=np.array([np.inf, np.inf, np.inf]))    # 1.5
            ...
            if USE_OBS_IN:
                ...
                obs_rho_i = uavs[i].obs_att.observe(dy=syst_dynamic, de=de_rho_i)
            else:
    ...
    if SAVE:
        ...
        for i in range(g_v['uav_num']):
            ...
            uavs[i].obs_pos.save_adap_obs_param(path, flag='pos')
            tt = np.linspace(0, g_v['g_tm'], int(g_v['g_tm'] / g_v['dt']))
            pd.DataFrame(REF, columns=['center_x', 'center_y', 'center_z', 'psi']). \
                to_csv(path + 'geometric_center.csv', sep=',', index=False)

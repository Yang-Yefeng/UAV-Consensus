...
from observer.PredefinedTimeDO import predefined_time_do
from controller.PdT_FNTSMC import pdt_fntsmc_param, pdt_fntsmc
...
att_ctrl_param = pdt_fntsmc_param(
    ...
    k2=np.array([1, 1, 1]).astype(float),  # 补偿观测器
    dim=3,
    dt=DT
)
'''Parameter list of the attitude controller'''
...
if __name__ == '__main__':
    uav = UAV(uav_param)
    ctrl_in = pdt_fntsmc(att_ctrl_param)
    ctrl_out = pdt_fntsmc(pos_ctrl_param)
    ...
    '''observer'''
    obs_in = predefined_time_do(T0=np.array([1, 1, 1]).astype(float),
                         ...
                         dim=3,
                         dt=uav.dt)
    obs_out = predefined_time_do(T0=np.array([1, 1, 1]).astype(float),
                          k1=np.array([1, 1, 1]).astype(float),
                          alpha=np.array([0.5, 0.5, 0.5]).astype(float),
                          beta1=np.array([2, 2, 2]).astype(float),
                          beta2=np.array([1, 1, 1]).astype(float),
    ...
    while uav.time < uav.time_max - uav.dt / 2:
        ...
        phi_d, theta_d, dot_phi_d, dot_theta_d, throttle = uo_2_ref_angle_throttle(uo=ctrl_out.control_out,
                                                                                   ...
                                                                                   dt=uav.dt,
                                                                                   att_limit=[np.pi/3, np.pi/3, np.pi/3],
                                                                                   dot_att_limit=None)  # [np.pi / 2, np.pi / 2, np.pi / 2]) 就极其不好

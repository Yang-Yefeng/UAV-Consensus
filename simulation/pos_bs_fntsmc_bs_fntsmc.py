...
if __name__ == '__main__':
    ...
    while uav.time < uav.time_max - uav.dt / 2:
        ...
        ref, dot_ref, dot2_ref, dot3_ref = ref_uav(uav.time, ref_amplitude, ref_period, ref_bias_a, ref_bias_phase)
    ...
    if SAVE:
        if windows:
            new_path = cur_path + '\\..\\datasave\\pos_bs_fntsmc-' + datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d-%H-%M-%S') + '/'
        else:
            new_path = cur_path + '/../datasave/pos_bs_fntsmc-' + datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d-%H-%M-%S') + '/'
        os.mkdir(new_path)
